...
# phase.tag <- c(rep("Money_Decision", 4*timec*young.num), rep("Money_Decision", 4*timec*old.num))
...
ROI_try <- cbind(ROI_try, age.tag, cond.tag, tc.tag, sub.tag)
tydi.ROI <- gather(ROI_try, ROI_try, value, -age.tag, -cond.tag, -tc.tag, -sub.tag)
...
young.phase1.tb <- c()
young.phase2.tb <- c()
T.mean <- tapply(tydi.ROI$value, list(tydi.ROI$tc.tag, tydi.ROI$age.tag), mean)
...
for(i in 1:4){
  sub.mean.df <- aggregate(tydi.ROI$value, list(tydi.ROI$sub.tag, tydi.ROI$cond.tag, tydi.ROI$age.tag), mean)
  colnames(sub.mean.df) <- c("sub.tag", "sit", "group", "signalvalue")
  
  sub.mean.df <- sub.mean.df[sub.mean.df$sit==i,]
