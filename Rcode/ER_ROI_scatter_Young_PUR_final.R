...
ROI_try <- read.csv("ER_diff_roi_final_young_pur.csv", header = T)
  ...
  for (jj in 1:length(levels(tydi.ROI$phase.tag))){
    ...
    for (i in analysis.factor){
      ...
      Y.pur.er.list[[out.iter]] <- ggscatter(ER.ROI.final, x = colnames(ER.ROI.final)[i], y = "signal_value", 
        ...
        annotate(geom="text", x=hjustvalue[iter], y=7.5, col=c("#C6922C"), 
                 label=paste("Young: r =", 
        ...
        annotate(geom="text", x=hjustvalue[iter], y=6.5, col=c("#3A5BA0"), 
                 label=paste("Old: r =", 
                             ...
                             ")"), 
                 size = size.pro, hjust = 1) +
        annotate(geom="text", x=hjustvalue[iter], y=5.5, col="black", 
                 label=paste("All: r =", 
...
Y.pur.er.list.g.final <- ggarrange(Y.pur.er.list.g[[1]],
                                   Y.pur.er.list.g[[2]],
                                   Y.pur.er.list.g[[3]],
                                   Y.pur.er.list.g[[4]],
                                   Y.pur.er.list.g[[5]],
...
jpeg(file = "y_PUR_ER.jpg", width = 7200, height = 4000)
print(Y.pur.er.list.g.final)
dev.off()
