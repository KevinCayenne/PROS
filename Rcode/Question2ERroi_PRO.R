...
for (er.factor.iter in c(1:7)){
  ...
  PROPURorPURNEU.set <- 1 # 1 (PROPUR) or 2(PURNEU)
  ...
  jpeg(file = paste("factor_", fac.title[er.factor.iter], "_PRO_EmoRating_ROI.jpg"), width = 5000, height = 1300)
  print(EMO.temp.K)
  dev.off()
...
jpeg(file = paste("PRO_Ques2ER_beta.jpg"), width = 2000, height = 800)
