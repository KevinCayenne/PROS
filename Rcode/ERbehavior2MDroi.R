...
####
ER.PRO.id <- ER.temp.PP[ER.temp.PP$sit.tag == "PRO" & ER.temp.PP$reg.tag == er.factor.iter,]
...
for (j in analysis.col){
  ...
  new.corrmerge <- cbind(new.corrmerge, Mean_emotion_rating = ER.PRO.id.aggre$mean.emo) 
  ...
  ####
  Y.lm <- summary(lm(new.corrmerge[new.corrmerge$age.tag == 1,]$Mean_emotion_rating ~ new.corrmerge[new.corrmerge$age.tag == 1,]$signalvalue))
  O.lm <- summary(lm(new.corrmerge[new.corrmerge$age.tag == 2,]$Mean_emotion_rating ~ new.corrmerge[new.corrmerge$age.tag == 2,]$signalvalue))
  All.lm <- summary(lm(new.corrmerge$Mean_emotion_rating~ new.corrmerge$signalvalue))
  
  cor.test.pro.Y <- cor.test(new.corrmerge[new.corrmerge$age.tag == 1,]$Mean_emotion_rating, new.corrmerge[new.corrmerge$age.tag == 1,]$signalvalue)
  cor.test.pro.O <- cor.test(new.corrmerge[new.corrmerge$age.tag == 2,]$Mean_emotion_rating, new.corrmerge[new.corrmerge$age.tag == 2,]$signalvalue)
  cor.test.pro <- cor.test(new.corrmerge$Mean_emotion_rating, new.corrmerge$signalvalue)
  
  #### ploting
  pros.emo.rating.scatter[[iter]] <- ggscatter(new.corrmerge, x = "Mean_emotion_rating", y = "signalvalue", 
                                     color = "Groups",
                                     palette = c("#C6922C","#3A5BA0"),
                                     xlab = "Mean emotion reaction", ylab = "Parameter Estimate (a.u.)",
                                     size = 5
  ) + 
    ...
    annotate(geom="text", x=4, y=4.5, col=c("#C6922C"), 
             label=paste("Young: r =", 
    ...
    annotate(geom="text", x=4, y=4, col=c("#3A5BA0"), 
             label=paste("Old: r =", 
                         ...
                         ")"), 
             size = size.pro, hjust = 1) +
    annotate(geom="text", x=4, y=3.5, col="black", 
             label=paste("All: r =", 
...
jpeg(file = paste("factor_", fac.title[er.factor.iter], "_PROPUR_EmoRating_ROI.jpg"), width = 2000, height = 1200)
print(EMO.temp.K)
dev.off()
